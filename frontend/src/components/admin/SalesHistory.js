...
export default function SalesHistory() {
  ...
  const filteredSales = sales.filter(s => 
    (s.vendedor?.nombre && s.vendedor.nombre.toLowerCase().includes(searchTerm.toLowerCase())) ||
      ...
      <div className="glass-card p-6 rounded-3xl border border-primary/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h3 className="font-bold text-white text-lg flex items-center gap-2">
            <Receipt className="w-5 h-5 text-primary" /> Historial de Ventas
          </h3>
          <p className="text-sm text-neutral-content/60">{sales.length} ventas registradas</p>
        </div>
          ...
          <tbody>
            {filteredSales.map(s => (
              <tr key={s.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                ...
                <td className="p-4 font-bold text-white text-sm">{s.vendedor?.nombre || "N/A"}</td>
                <td className="p-4 text-sm">{s.cliente?.nombre || "Venta Rápida"}</td>
                <td className="p-4 text-sm text-primary font-bold">{s.vape?.nombre}</td>
            ...
            {filteredSales.length === 0 && (
              <tr><td colSpan="8" className="p-8 text-center text-neutral-content/50">No hay ventas registradas.</td></tr>
            )}
          </tbody>
        </table>
      </div>
